...
[MQL5 Reference](index.md)  /  [Network Functions](network.md) / SocketTlsRead

[![Previous](previous.png)](sockettlscertificate.md) 
[![Next](next.png)](sockettlsreadavailable.md)
...
[in]  Number of bytes to read to the buffer[] array. Data not fitting into the array remain in the socket. They can be received by the next SocketTLSRead call. buffer\_maxlen cannot exceed [INT\_MAX](typeconstants.md) (2147483647).
...
The function is executed till it receives the specified amount of data or the timeout is reached ([SocketTimeouts](sockettimeouts.md)).
