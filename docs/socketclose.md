...
[MQL5 Reference](index.md)  /  [Network Functions](network.md) / SocketClose

[![Previous](previous.png)](socketcreate.md) 
[![Next](next.png)](socketconnect.md)
...
[in]  Handle of a socket to be closed. The handle is returned by the [SocketCreate](socketcreate.md) function. When an incorrect handle is passed, the error 5270 (ERR\_NETSOCKET\_INVALIDHANDLE) is written to [\_LastError](_lasterror.md).
...
If a connection via [SocketConnect](socketconnect.md) was previously created for a socket, it is discontinued.
