...
[MQL5 Reference](index.md)  /  [Event Handling](event_handlers.md) / OnTick

[![Previous](previous.png)](ondeinit.md) 
[![Next](next.png)](oncalculate.md)

OnTick

The function is called in EAs when the [NewTick](event_fire.md#newtick) event occurs to handle a new quote.
...
The [NewTick](event_fire.md#newtick) event is generated only for EAs upon receiving a new tick for a symbol of the chart the EA is attached to. There is no point in defining the OnTick() function in a custom indicator or a script since a NewTick event is not generated for them.
...
All events are handled one after another in the order of their receipt. If the queue already contains the [NewTick](event_fire.md#newtick) event or this event is in the processing stage, then the new NewTick event is not added to mql5 application queue.
...
[Event handling functions](events.md), [Program running](running.md), [Client terminal events](event_fire.md), [OnTimer](ontimer.md), [OnBookEvent](onbookevent.md), [OnChartEvent](onchartevent.md)