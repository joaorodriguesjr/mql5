...
[MQL5 Reference](index.md)  /  [Timeseries and Indicators Access](series.md) / CopyOpen

[![Previous](previous.png)](copytime.md) 
[![Next](next.png)](copyhigh.md)

CopyOpen

The function gets into open\_array the history data of bar open prices for the selected symbol-period pair in the specified quantity. It should be noted that elements ordering is from present to past, i.e., starting position of 0 means the current bar.

![CopyOpen](copyopen.png)
...
Returns the number of element in the array or -1 in case of an [error](errorcodes.md).
